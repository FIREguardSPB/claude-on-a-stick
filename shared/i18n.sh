...
MSG[en.setup_token_launch_hint]="A browser window will open. Log in, then setup-token will PRINT a long token (sk-ant-oat...) right here in this terminal. COPY that whole token, return here, and paste it at the prompt below."
...
MSG[ru.setup_token_launch_hint]="Откроется окно браузера. Войдите в систему, после чего setup-token РАСПЕЧАТАЕТ длинный токен (sk-ant-oat...) прямо здесь, в этом терминале. СКОПИРУЙТЕ этот токен целиком, вернитесь сюда и вставьте его в приглашение ниже."
